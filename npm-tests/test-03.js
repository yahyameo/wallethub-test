...
const dataString = JSON.parse(json).data;

// Function to extract key-value pairs
function parseString(dataString) {
  return dataString.split(", ").map((item) => item.split("=")[0]);
}

// Count items with age 32
const ageCount = dataString.split(", ").filter((item) => {
  const [, age] = item.split("=");
  return parseInt(age) === 32;
}).length;

console.log(`Number of items with age 32: ${ageCount}`);

// Extract and write key values to a file
const keyValues = parseString(dataString);
...
writeStream.on("finish", () => console.log("Key values written to output.txt"));
